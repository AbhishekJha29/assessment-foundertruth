 ...
 * In the unified Next.js setup, defaults to same-origin relative path '/api/v1'.
 */
const getApiBaseUrl = () => {
  let base = process.env.NEXT_PUBLIC_API_BASE_URL || '/api/v1';

  // Remove trailing slashes
  base = base.replace(/\/+$/, '');

  // If user entered only domain (e.g. https://my-domain.vercel.app), append /api/v1
...
export const apiFetch = async (endpoint, options = {}) => {
  ...
  try {
    ...
    throw new Error(
      err.message || `Unable to connect to the backend at ${API_BASE_URL}.`
    );
  }
};
...
export const feedApi = {
  getFeed: async ({ page = 1, limit = 9, sort = 'latest', source = '', search = '' } = {}) => {
